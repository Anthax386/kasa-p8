...
function Collapse ({collapse}) {
    ...
    async function openCloseCollapse() {

        const arrow = document.getElementById(collapse.name)
        const txt = document.getElementById(collapse.name+'text')
    ...
    return (
        <article key={collapse.name} className='dropdown'>
            ...
            <div className="dropdown-text hidden" id={collapse.name+'text'}>
                <p>{collapse.txt}</p>
            </div>
        </article>
    )
